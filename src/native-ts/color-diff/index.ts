...
function getEmitterWithRootNode(
  result: unknown,
): { rootNode: HljsNode } | null {
  if (typeof result !== 'object' || result === null) {
    return null
  }
  const candidates = [
    'emitter' in result ? result.emitter : undefined,
    '_emitter' in result ? result._emitter : undefined,
  ]
  for (const candidate of candidates) {
    if (hasRootNode(candidate)) return candidate
  }
  return null
}
  ...
  if (!emitter) {
    if (!loggedEmitterShapeError) {
      ...
      const emitterKeys =
        ...
        result !== null &&
        'emitter' in result &&
        typeof result.emitter === 'object' &&
        result.emitter !== null
          ? Object.keys(result.emitter)
          : []
      const legacyEmitterKeys =
        typeof result === 'object' &&
        result !== null &&
        '_emitter' in result &&
        typeof result._emitter === 'object' &&
        result._emitter !== null
          ? Object.keys(result._emitter)
          : []
      logError(
        new Error(
          `color-diff: hljs emitter shape mismatch (result keys: ${resultKeys.join(',')}; emitter keys: ${emitterKeys.join(',')}; _emitter keys: ${legacyEmitterKeys.join(',')}). Syntax highlighting disabled.`,
        ),
      )
    }
    return [[defaultStyle(theme), code]]
