...
export async function cleanupNpmInstallations(): Promise<{
  ...
  // Always attempt to remove semiclaw-code
  const codePackageResult = await attemptNpmUninstall('semiclaw-code')
  ...
  if (MACRO.PACKAGE_URL && MACRO.PACKAGE_URL !== 'semiclaw-code') {
